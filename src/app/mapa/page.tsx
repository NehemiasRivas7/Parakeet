...
export default async function MapaPage({
  ...
  searchParams: Promise<{ zona?: string; ok?: string; recuperada?: string }>;
}) {
  const sp = await searchParams;
  const reporteEnviado = sp.ok === '1';
  const zonaRecuperada = sp.recuperada === '1';
  ...
  return (
    <main className="flex h-dvh flex-col">
      ...
      {zonaRecuperada && (
        <div className="mx-4 mt-3 rounded-xl border border-brand bg-brand px-3 py-2 text-sm font-semibold text-white shadow-[0_10px_22px_-10px_rgba(16,148,45,0.6)]">
          ¡Jornada cerrada! La zona bajó de nivel — mirá el cambio en el mapa.
        </div>
      )}
